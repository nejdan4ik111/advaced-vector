...
public:
    ...
    iterator EmplaceWithReallocationAndShift(size_t shift, Args && ...args);

    size_t Size() const noexcept;
    size_t Capacity() const noexcept;
    const T& operator[](size_t index) const noexcept;
    T& operator[](size_t index) noexcept;
    T* EmplaceWithReallocation(size_t shift, Args && ...args);
...
inline T& Vector<T>::EmplaceBack(Args && ...args) {
    // Убираем задвоение проверки типа
    T* result = nullptr;
    if (size_ == Capacity()) {
        // Вместо дублирования кода, вызываем вспомогательный метод
        result = EmplaceWithReallocation(size_, std::forward<Args>(args)...);
    }
    else {
...
typename Vector<T>::iterator Vector<T>::Emplace(const_iterator pos, Args && ...args) {
    ...
    if (size_ == Capacity()) {
        // Вместо дублирования кода, вызываем вспомогательный метод
        return EmplaceWithReallocationAndShift(shift, std::forward<Args>(args)...);
    }
    else {
        // Оставляем текущую реализацию
        if (size_ != 0) {
...
template<typename T>
template<typename ...Args>
T* Vector<T>::EmplaceWithReallocation(size_t shift, Args && ...args) {
    size_t new_capacity = size_ == 0 ? 1 : size_ * 2;
    RawMemory<T> new_data(new_capacity);

    iterator result = new (new_data.GetAddress() + shift) T(std::forward<Args>(args)...);
...
template<typename T>
template<typename ...Args>
typename Vector<T>::iterator Vector<T>::EmplaceWithReallocationAndShift(size_t shift, Args && ...args) {
    T* result = EmplaceWithReallocation(shift, std::forward<Args>(args)...);

    // Передвигаем элементы сдвига вправо
    try {
